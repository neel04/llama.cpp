...
void ggml_bf16_to_fp32_row(const ggml_bf16_t *x, float *y, int64_t n) {
    ...
    for (; i + 8 <= n; i += 8) {
        _mm256_storeu_ps(y + i,
                         _mm256_castsi256_ps(
                             _mm256_slli_epi32(_mm256_cvtepu16_epi32(_mm_loadu_si128((const __m128i *)(x + i))), 16)));
    }
#endif
    for (; i < n; i++) {
...
static void ggml_vec_dot_f32(int n,
                             float *restrict s,
                             size_t bs,
                             ...
                             size_t by,
                             int nrc);
static void ggml_vec_dot_f16(int n,
                             float *restrict s,
                             size_t bs,
                             ...
                             size_t by,
                             int nrc);
static void ggml_vec_dot_bf16(int n,
                              float *restrict s,
                              size_t bs,
                              ggml_bf16_t *restrict x,
                              size_t bx,
                              ggml_bf16_t *restrict y,
                              size_t by,
                              int nrc);
...
inline static void
ggml_vec_mad_f16(const int n, ggml_fp16_t *restrict y, const ggml_fp16_t *restrict x, const float v) {
#if defined(GGML_SIMD)
    const int np = (n & ~(GGML_F16_STEP - 1));
...
inline static void ggml_vec_mad_f32_unroll(
    const int n, const int xs, const int vs, float *restrict y, const float *restrict xv, const float *restrict vv) {
...
inline static float32x4_t ggml_v_expf(float32x4_t x) {
    ...
    return vbslq_f32(vcagtq_f32(n, vdupq_n_f32(192)),
                     vmulq_f32(s1, s1),
...
inline static __m512 ggml_v_expf(__m512 x) {
    ...
    const __m512 j = _mm512_fmadd_ps(
        _mm512_fmadd_ps(_mm512_fmadd_ps(_mm512_set1_ps(0x1.0e4020p-7f), b, _mm512_set1_ps(0x1.573e2ep-5f)),
                        u,
...
inline static __m256 ggml_v_expf(__m256 x) {
    ...
    const __m256 j = _mm256_fmadd_ps(
        _mm256_fmadd_ps(_mm256_fmadd_ps(_mm256_set1_ps(0x1.0e4020p-7f), b, _mm256_set1_ps(0x1.573e2ep-5f)),
                        u,
...
inline static __m128 ggml_v_expf(__m128 x) {
    ...
    const __m128 j = MADD128(MADD128(MADD128(_mm_set1_ps(0x1.0e4020p-7f), b, _mm_set1_ps(0x1.573e2ep-5f)),
                                     u,
...
struct ggml_tensor *
ggml_new_tensor_3d(struct ggml_context *ctx, enum ggml_type type, int64_t ne0, int64_t ne1, int64_t ne2) {
    const int64_t ne[3] = {ne0, ne1, ne2};
    return ggml_new_tensor(ctx, type, 3, ne);
}

struct ggml_tensor *
ggml_new_tensor_4d(struct ggml_context *ctx, enum ggml_type type, int64_t ne0, int64_t ne1, int64_t ne2, int64_t ne3) {
...
void ggml_unravel_index(
    const struct ggml_tensor *tensor, int64_t i, int64_t *i0, int64_t *i1, int64_t *i2, int64_t *i3) {
...
ggml_add_impl(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, bool inplace) {
...
static struct ggml_tensor *
ggml_add_cast_impl(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, enum ggml_type type) {
...
struct ggml_tensor *
ggml_add_cast(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, enum ggml_type type) {
...
static struct ggml_tensor *
ggml_add1_impl(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, bool inplace) {
...
static struct ggml_tensor *
ggml_sub_impl(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, bool inplace) {
    GGML_ASSERT(ggml_can_repeat(b, a));
...
static struct ggml_tensor *
ggml_mul_impl(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, bool inplace) {
    GGML_ASSERT(ggml_can_repeat(b, a));
...
static struct ggml_tensor *
ggml_div_impl(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, bool inplace) {
    GGML_ASSERT(ggml_can_repeat(b, a));
...
struct ggml_tensor *
ggml_leaky_relu(struct ggml_context *ctx, struct ggml_tensor *a, float negative_slope, bool inplace) {
    struct ggml_tensor *result = inplace ? ggml_view_tensor(ctx, a) : ggml_dup_tensor(ctx, a);
...
static struct ggml_tensor *
ggml_rms_norm_impl(struct ggml_context *ctx, struct ggml_tensor *a, float eps, bool inplace) {
    struct ggml_tensor *result = inplace ? ggml_view_tensor(ctx, a) : ggml_dup_tensor(ctx, a);
...
struct ggml_tensor *
ggml_rms_norm_back(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, float eps) {
    struct ggml_tensor *result = ggml_dup_tensor(ctx, a);
...
static struct ggml_tensor *
ggml_group_norm_impl(struct ggml_context *ctx, struct ggml_tensor *a, int n_groups, float eps, bool inplace) {
    struct ggml_tensor *result = inplace ? ggml_view_tensor(ctx, a) : ggml_dup_tensor(ctx, a);
...
*/
struct ggml_tensor *
ggml_mul_mat_id(struct ggml_context *ctx, struct ggml_tensor *as, struct ggml_tensor *b, struct ggml_tensor *ids) {
...
struct ggml_tensor *
ggml_set_1d_inplace(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, size_t offset) {
    return ggml_set_impl(ctx, a, b, a->nb[1], a->nb[2], a->nb[3], offset, true);
}

struct ggml_tensor *
ggml_set_2d(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, size_t nb1, size_t offset) {
    return ggml_set_impl(ctx, a, b, nb1, a->nb[2], a->nb[3], offset, false);
}

struct ggml_tensor *
ggml_set_2d_inplace(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, size_t nb1, size_t offset) {
...
ggml_cont_3d(struct ggml_context *ctx, struct ggml_tensor *a, int64_t ne0, int64_t ne1, int64_t ne2) {
    return ggml_cont_4d(ctx, a, ne0, ne1, ne2, 1);
}

struct ggml_tensor *
ggml_cont_4d(struct ggml_context *ctx, struct ggml_tensor *a, int64_t ne0, int64_t ne1, int64_t ne2, int64_t ne3) {
...
struct ggml_tensor *
ggml_reshape_3d(struct ggml_context *ctx, struct ggml_tensor *a, int64_t ne0, int64_t ne1, int64_t ne2) {
    GGML_ASSERT(ggml_is_contiguous(a));
...
struct ggml_tensor *
ggml_reshape_4d(struct ggml_context *ctx, struct ggml_tensor *a, int64_t ne0, int64_t ne1, int64_t ne2, int64_t ne3) {
    GGML_ASSERT(ggml_is_contiguous(a));
    GGML_ASSERT(ggml_nelements(a) == ne0 * ne1 * ne2 * ne3);
...
static struct ggml_tensor *
ggml_view_impl(struct ggml_context *ctx, struct ggml_tensor *a, int n_dims, const int64_t *ne, size_t offset) {
...
struct ggml_tensor *
ggml_view_2d(struct ggml_context *ctx, struct ggml_tensor *a, int64_t ne0, int64_t ne1, size_t nb1, size_t offset) {
    const int64_t ne[2] = {ne0, ne1};
...
struct ggml_tensor *
ggml_permute(struct ggml_context *ctx, struct ggml_tensor *a, int axis0, int axis1, int axis2, int axis3) {
...
struct ggml_tensor *
ggml_get_rows_back(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, struct ggml_tensor *c) {
...
static struct ggml_tensor *
ggml_diag_mask_inf_impl(struct ggml_context *ctx, struct ggml_tensor *a, int n_past, bool inplace) {
    struct ggml_tensor *result = inplace ? ggml_view_tensor(ctx, a) : ggml_dup_tensor(ctx, a);
...
static struct ggml_tensor *
ggml_diag_mask_zero_impl(struct ggml_context *ctx, struct ggml_tensor *a, int n_past, bool inplace) {
    struct ggml_tensor *result = inplace ? ggml_view_tensor(ctx, a) : ggml_dup_tensor(ctx, a);
...
struct ggml_tensor *ggml_soft_max_ext(
    struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *mask, float scale, float max_bias) {
...
static struct ggml_tensor *
ggml_soft_max_back_impl(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, bool inplace) {
    struct ggml_tensor *result = inplace ? ggml_view_tensor(ctx, a) : ggml_dup_tensor(ctx, a);
...
struct ggml_tensor *
ggml_rope(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, int n_dims, int mode) {
    return ggml_rope_impl(ctx, a, b, NULL, n_dims, mode, 0, 10000.0f, 1.0f, 0.0f, 1.0f, 0.0f, 0.0f, false);
}

struct ggml_tensor *
ggml_rope_inplace(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, int n_dims, int mode) {
...
GGML_API struct ggml_tensor *
ggml_conv_1d(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, int s0, int p0, int d0) {
    struct ggml_tensor *im2col = ggml_im2col(ctx, a, b, s0, 0, p0, 0, d0, 0, false, GGML_TYPE_F16); // [N, OL, IC * K]

    struct ggml_tensor *result = ggml_mul_mat(
        ctx,
        ggml_reshape_2d(
            ctx, im2col, im2col->ne[0], (im2col->ne[2] * im2col->ne[1])), // [N, OL, IC * K] => [N*OL, IC * K]
...
struct ggml_tensor *
ggml_conv_1d_ph(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, int s, int d) {
...
GGML_API struct ggml_tensor *
ggml_conv_transpose_1d(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, int s0, int p0, int d0) {
...
struct ggml_tensor *ggml_conv_depthwise_2d(struct ggml_context *ctx,
    ...
    struct ggml_tensor *new_b = ggml_reshape_4d(ctx,
                                                im2col,
                                                im2col->ne[0],
...
struct ggml_tensor *
ggml_conv_transpose_2d_p0(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, int stride) {
...
struct ggml_tensor *
ggml_pool_1d(struct ggml_context *ctx, struct ggml_tensor *a, enum ggml_op_pool op, int k0, int s0, int p0) {
    const int64_t ne[4] = {
...
static struct ggml_tensor *
ggml_upscale_impl(struct ggml_context *ctx, struct ggml_tensor *a, int ne0, int ne1, int ne2, int ne3) {
...
struct ggml_tensor *
ggml_upscale_ext(struct ggml_context *ctx, struct ggml_tensor *a, int ne0, int ne1, int ne2, int ne3) {
...
struct ggml_tensor *
ggml_timestep_embedding(struct ggml_context *ctx, struct ggml_tensor *timesteps, int dim, int max_period) {
    int actual_dim = dim;
    if (dim % 2 != 0) {
        actual_dim = dim + 1;
    }
...
struct ggml_tensor *ggml_top_k(struct ggml_context *ctx, struct ggml_tensor *a, int k) {
    ...
    result = ggml_view_4d(
        ctx, result, k, result->ne[1], result->ne[2], result->ne[3], result->nb[1], result->nb[2], result->nb[3], 0);
...
struct ggml_tensor *ggml_flash_attn_ext(struct ggml_context *ctx,
    ...
    if (mask) {
        ...
        GGML_ASSERT(mask->ne[1] >= GGML_PAD(q->ne[1], GGML_KQ_MASK_PAD)
                    && "the Flash-Attention kernel requires the mask to be padded to GGML_KQ_MASK_PAD and at least "
                       "n_queries big");
...
static struct ggml_tensor *ggml_add_rel_pos_impl(
    struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *pw, struct ggml_tensor *ph, bool inplace) {
...
struct ggml_tensor *
ggml_add_rel_pos(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *pw, struct ggml_tensor *ph) {
...
static struct ggml_tensor *
ggml_unary_impl(struct ggml_context *ctx, struct ggml_tensor *a, enum ggml_unary_op op, bool inplace) {
...
static struct ggml_tensor *
ggml_map_unary_impl_f32(struct ggml_context *ctx, struct ggml_tensor *a, const ggml_unary_op_f32_t fun, bool inplace) {
    struct ggml_tensor *result = inplace ? ggml_view_tensor(ctx, a) : ggml_dup_tensor(ctx, a);
...
struct ggml_tensor *
ggml_map_unary_inplace_f32(struct ggml_context *ctx, struct ggml_tensor *a, const ggml_unary_op_f32_t fun) {
    return ggml_map_unary_impl_f32(ctx, a, fun, true);
}

// ggml_map_binary

static struct ggml_tensor *ggml_map_binary_impl_f32(struct ggml_context *ctx,
                                                    struct ggml_tensor *a,
                                                    struct ggml_tensor *b,
...
struct ggml_tensor *ggml_map_binary_inplace_f32(struct ggml_context *ctx,
                                                struct ggml_tensor *a,
                                                struct ggml_tensor *b,
                                                const ggml_binary_op_f32_t fun) {
...
struct ggml_tensor *
ggml_map_custom1_f32(struct ggml_context *ctx, struct ggml_tensor *a, const ggml_custom1_op_f32_t fun) {
    return ggml_map_custom1_impl_f32(ctx, a, fun, false);
}

struct ggml_tensor *
ggml_map_custom1_inplace_f32(struct ggml_context *ctx, struct ggml_tensor *a, const ggml_custom1_op_f32_t fun) {
...
struct ggml_tensor *ggml_map_custom2_inplace_f32(struct ggml_context *ctx,
                                                 struct ggml_tensor *a,
                                                 struct ggml_tensor *b,
                                                 const ggml_custom2_op_f32_t fun) {
...
struct ggml_tensor *ggml_map_custom3_inplace_f32(struct ggml_context *ctx,
                                                 struct ggml_tensor *a,
                                                 struct ggml_tensor *b,
                                                 struct ggml_tensor *c,
                                                 const ggml_custom3_op_f32_t fun) {
...
struct ggml_tensor *ggml_map_custom1(
    struct ggml_context *ctx, struct ggml_tensor *a, const ggml_custom1_op_t fun, int n_tasks, void *userdata) {
...
#ifdef GGML_USE_ACCELERATE
        vDSP_vadd((float *)((char *)src0->data + i3 * nb03 + i2 * nb02 + i1 * nb01 + offset),
                  1,
        ...
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            for (int64_t i01 = 0; i01 < ne01; i01++) {
                ggml_vec_sum_f32_ggf(
                    ne00, &row_sum, (float *)((char *)src0->data + i01 * nb01 + i02 * nb02 + i03 * nb03));
                sum += row_sum;
            }
        }
    }
        ...
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            for (int64_t i01 = 0; i01 < ne01; i01++) {
                ggml_vec_sum_f16_ggf(
                    ne00, &row_sum, (ggml_fp16_t *)((char *)src0->data + i01 * nb01 + i02 * nb02 + i03 * nb03));
                sum += row_sum;
            }
        }
    }
        ...
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            for (int64_t i01 = 0; i01 < ne01; i01++) {
                ggml_vec_sum_bf16_ggf(
                    ne00, &row_sum, (ggml_bf16_t *)((char *)src0->data + i01 * nb01 + i02 * nb02 + i03 * nb03));
                sum += row_sum;
            }
        }
    }
...
static void ggml_compute_forward_gelu_f32(const struct ggml_compute_params *params, struct ggml_tensor *dst) {
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ggml_vec_gelu_f32(
            nc, (float *)((char *)dst->data + i1 * (dst->nb[1])), (float *)((char *)src0->data + i1 * (src0->nb[1])));
...
//     if (src1->ne[1] <= 1 && src0->type != GGML_TYPE_TL1 && src0->type != GGML_TYPE_I2_S && src0->type !=
//     GGML_TYPE_TQ1_0 && src0->type != GGML_TYPE_TQ2_0 && src0->ne[1] != 32002 && src0->ne[1] != 96 && src0->ne[0] !=
//     96) {
            ...
            for (int i = 0; i < bs128_num; i++) {
                ...
                for (int i_tile = th_tile_beg; i_tile < th_tile_end; i_tile++) {
                    ...
                    ggml_qgemm_lut(128,
                                   ...
                                   two_A + two_w_offset,
                                   NULL,
                                   two_qlut + bs512_num * 512 * two_k / 2 * 32 + bs256_num * 256 * two_k / 2 * 32
                                       + i * 128 * two_k / 2 * 32,
                                   wt->scales,
                                   lut_scales + bs512_num * 512 + bs256_num * 256 + i * 128,
                ...
                for (int i_tile = th_tile_beg; i_tile < th_tile_end; i_tile++) {
                    ...
                    ggml_qgemm_lut(8,
                                   ...
                                   three_qlut + bs512_num * 512 * three_k / 3 * 32 + bs256_num * 256 * three_k / 3 * 32
                                       ...
                                       + i * 8 * three_k / 3 * 32,
                                   wt->scales,
                                   lut_scales + bs512_num * 512 + bs256_num * 256 + bs128_num * 128 + bs32_num * 32
                                       + i * 8,
                                   act_output + dst_offset + bs512_num * 512 * ne01 + bs256_num * 256 * ne01
                                       + bs128_num * 128 * ne01 + bs32_num * 32 * ne01 + i * 8 * ne01);
                }
                ...
                for (int i_tile = th_tile_beg; i_tile < th_tile_end; i_tile++) {
                    ...
                    ggml_qgemm_lut(1,
                                   ...
                                   lut_scales + bs512_num * 512 + bs256_num * 256 + bs128_num * 128 + bs32_num * 32
                                       + bs8_num * 8 + i * 1,
                                   act_output + dst_offset + bs512_num * 512 * ne01 + bs256_num * 256 * ne01
                                       + bs128_num * 128 * ne01 + bs32_num * 32 * ne01 + bs8_num * 8 * ne01
                                       + i * 1 * ne01);
                }
        ...
        for (int64_t i13 = 0; i13 < ne13; ++i13) {
            for (int64_t i12 = 0; i12 < ne12; ++i12) {
                ...
                for (int64_t i11 = i11_processed + ith; i11 < ne11; i11 += nth) {
                    if (src0->type == GGML_TYPE_I2_S) {
                        quantize_row_i8_s((float *)((char *)src1->data + i13 * nb13 + i12 * nb12 + i11 * nb11),
                                          (void *)(wdata + i13 * nbw3 + i12 * nbw2 + i11 * nbw1),
                                          ne10,
                                          act_scales + i11,
                                          act_sums + i11);
...
UseGgmlGemm2:;
    ...
    if ((ggml_n_dims(src0) == 2) && gemv) {
        ...
        const size_t src1_col_stride =
            ggml_is_contiguous(src1) || src1->type != vec_dot_type ? ggml_row_size(vec_dot_type, ne10) : nb11;
        int64_t src0_start = (ith * ne01) / nth;
        int64_t src0_end = ((ith + 1) * ne01) / nth;
        src0_start =
            (src0_start % matmul_num_cols) ? src0_start + matmul_num_cols - (src0_start % matmul_num_cols) : src0_start;
        ...
        if (gemm && (ne11 > 3)) {
            gemm(ne00,
                 (float *)((char *)dst->data) + src0_start,
                 ne01,
        ...
        for (int iter = gemm ? ne11 - ne11 % 4 : 0; iter < ne11; iter++) {
            gemv(ne00,
                 (float *)((char *)dst->data + (iter * nb1)) + src0_start,
                 ne01,
                 (const char *)src0->data + src0_start * nb01,
    ...
    for (int cur_a = 0; cur_a < n_as; ++cur_a) {
        ...
        if (((ggml_n_dims(src0) - 1) == 2) && gemv) {
            ...
            src0_cur_start = (src0_cur_start % matmul_num_cols)
                                 ? src0_cur_start + matmul_num_cols - (src0_cur_start % matmul_num_cols)
                                 : src0_cur_start;
            src0_cur_end = (src0_cur_end % matmul_num_cols)
                               ? src0_cur_end + matmul_num_cols - (src0_cur_end % matmul_num_cols)
                               : src0_cur_end;
            ...
            for (int ir1 = 0; ir1 < nr1; ir1++) {
                ...
                const char *src1_col = (const char *)wdata
                                       + (src1_cont || src1->type != vec_dot_type ? (i11 + i12 * ne11) * row_size
                                                                                  : (i11 * nb11 + i12 * nb12));

                gemv(ne00,
                     (float *)((char *)dst->data + (i1 * nb1 + i2 * nb2)) + src0_cur_start,
                     ne01,
                     (const char *)src0_cur + src0_cur_start * nb01,
                     src1_col,
                     1,
        ...
        for (int64_t iir1 = ir110; iir1 < ir111; iir1 += blck_1) {
            for (int64_t iir0 = ir010; iir0 < ir011; iir0 += blck_0) {
                for (int64_t ir1 = iir1; ir1 < iir1 + blck_1 && ir1 < ir111; ++ir1) {
                    ...
                    // desc: when src1 is not a contiguous memory block we have to calculate the offset using the
                    // strides
...
static void
ggml_compute_forward_rope_f32(const struct ggml_compute_params *params, struct ggml_tensor *dst, const bool forward) {
    const struct ggml_tensor *src0 = dst->src[0];
    const struct ggml_tensor *src1 = dst->src[1];
    ...
    for (int64_t i3 = 0; i3 < ne3; i3++) {
        for (int64_t i2 = 0; i2 < ne2; i2++) {
            ...
            ggml_rope_cache_init(
                p, freq_scale, freq_factors, corr_dims, ne0, ext_factor, attn_factor, cache, sin_sign, theta_scale);
...
static void
ggml_compute_forward_rope_f16(const struct ggml_compute_params *params, struct ggml_tensor *dst, const bool forward) {
    const struct ggml_tensor *src0 = dst->src[0];
    const struct ggml_tensor *src1 = dst->src[1];
    const struct ggml_tensor *src2 = dst->src[2];
    ...
    for (int i1 = ir0; i1 < ir1; i1++) {
        ...
        for (int i10 = 0; i10 < ne10; i10++) {
            ...
            for (int i00 = 0; i00 < ne00; i00++) {
                float v = 0;
                ggml_vec_dot_f16(
                    ne02, &v, 0, (ggml_fp16_t *)wdata_src + i1n, 0, (ggml_fp16_t *)wdata_kernel + i00 * ne02, 0, 1);
                    ...
                    for (int64_t iiw = 0; iiw < IW; iiw++) {
                        ...
                        for (int64_t ikh = 0; ikh < KH; ikh++) {
                            for (int64_t ikw = 0; ikw < KW; ikw++) {
                                ...
                                // These values have tmpw % s0 != 0 and need to be skipped in the backwards pass as
                                // well.
            ...
            for (int i10 = 0; i10 < ne10; i10++) {
                ...
                for (int i01 = 0; i01 < ne01; i01++) {
                    for (int i00 = 0; i00 < ne00; i00++) {
                        float v = 0;
                        ggml_vec_dot_f16(
                            ne03, &v, 0, wdata_src + i1n, 0, wdata_kernel + i01 * ne00 * ne03 + i00 * ne03, 0, 1);
...
static void ggml_compute_forward_flash_attn_ext(const struct ggml_compute_params *params,
                                                const struct ggml_tensor *q,
                                                const struct ggml_tensor *k,
                                                const struct ggml_tensor *v,
                                                const struct ggml_tensor *mask,
...
static void ggml_compute_forward_rwkv_wkv_f32(const struct ggml_compute_params *params, struct ggml_tensor *dst) {
...
static void ggml_compute_forward_rwkv_wkv(const struct ggml_compute_params *params, struct ggml_tensor *dst) {
    const struct ggml_tensor *src0 = dst->src[0];
...
static void ggml_compute_forward_opt_step_adamw_f32(const struct ggml_compute_params *params, struct ggml_tensor *dst) {
    ...
    int64_t iter;
    memcpy(&iter, &dst->op_params[0], sizeof(int64_t));
...
static void ggml_compute_forward(struct ggml_compute_params *params, struct ggml_tensor *tensor) {
    ...
    case GGML_OP_FLASH_ATTN_EXT: {
        ggml_compute_forward_flash_attn_ext(
            params, tensor->src[0], tensor->src[1], tensor->src[2], tensor->src[3], tensor);
    } break;
...
size_t ggml_hash_size(size_t min_sz) {
    ...
    static const size_t primes[] = {
        ...
        2053,    4099,    8209,     16411,    32771,    65537,     131101,    262147,    524309,     1048583,   2097169,
        4194319, 8388617, 16777259, 33554467, 67108879, 134217757, 268435459, 536870923, 1073741827, 2147483659};
...
static struct ggml_tensor *ggml_recompute_graph_node(struct ggml_context *ctx,
    ...
    if (node->view_src != NULL) {
        clone->data = (node->view_src->data == NULL) ? NULL                         // view_src not yet allocated
...
void ggml_build_backward_gradient_checkpointing(struct ggml_context *ctx,
...
static struct ggml_tensor *ggml_acc_or_set(struct ggml_context *ctx,
    ...
    if (ggml_hash_contains(zero_table, a)) {
        struct ggml_tensor *a_zero =
            ggml_scale(ctx, a, 0.0f); // FIXME this is going to produce NaN if a contains inf/NaN
...
static void ggml_compute_backward(struct ggml_context *ctx,
    ...
    case GGML_OP_ADD: {
        ...
        if (src1->grad) {
            if (ggml_are_same_shape(src0, src1)) {
                ...
                src1->grad =
                    ggml_add_or_set(ctx, src1->grad, ggml_repeat_back(ctx, tensor->grad, src1), zero_table, acc_table);
            }
        }
    } break;
    case GGML_OP_ADD1: {
    ...
    case GGML_OP_MUL: {
        if (src0->grad) {
            src0->grad = ggml_add_or_set(ctx, src0->grad, ggml_mul(ctx, src1, tensor->grad), zero_table, acc_table);
        }
        if (src1->grad) {
            src1->grad = ggml_add_or_set(ctx, src1->grad, ggml_mul(ctx, src0, tensor->grad), zero_table, acc_table);
        }
    } break;
    case GGML_OP_DIV: {
        if (src0->grad) {
            src0->grad = ggml_add_or_set(ctx, src0->grad, ggml_div(ctx, tensor->grad, src1), zero_table, acc_table);
        }
        if (src1->grad) {
            src1->grad = ggml_sub_or_set(
                ctx, src1->grad, ggml_mul(ctx, tensor->grad, ggml_div(ctx, tensor, src1)), zero_table, acc_table);
        }
    } break;
    case GGML_OP_SQR: {
        if (src0->grad) {
            src0->grad = ggml_add_or_set(
                ctx, src0->grad, ggml_scale(ctx, ggml_mul(ctx, src0, tensor->grad), 2.0f), zero_table, acc_table);
        }
    } break;
    case GGML_OP_SQRT: {
        if (src0->grad) {
            src0->grad = ggml_add_or_set(
                ctx, src0->grad, ggml_scale(ctx, ggml_div(ctx, tensor->grad, tensor), 0.5f), zero_table, acc_table);
        }
    } break;
    case GGML_OP_LOG: {
        if (src0->grad) {
            src0->grad = ggml_add_or_set(ctx, src0->grad, ggml_div(ctx, tensor->grad, src0), zero_table, acc_table);
        }
    } break;
    case GGML_OP_SIN: {
        if (src0->grad) {
            src0->grad = ggml_add_or_set(
                ctx, src0->grad, ggml_mul(ctx, tensor->grad, ggml_cos(ctx, src0)), zero_table, acc_table);
        }
    } break;
    case GGML_OP_COS: {
        if (src0->grad) {
            src0->grad = ggml_sub_or_set(
                ctx, src0->grad, ggml_mul(ctx, tensor->grad, ggml_sin(ctx, src0)), zero_table, acc_table);
        }
    } break;
    case GGML_OP_SUM: {
        if (src0->grad) {
            src0->grad = ggml_add1_or_set(ctx, src0->grad, tensor->grad, zero_table, acc_table);
        }
    } break;
    case GGML_OP_SUM_ROWS: {
        if (src0->grad) {
            src0->grad =
                ggml_add_or_set(ctx, src0->grad, ggml_repeat(ctx, tensor->grad, src0->grad), zero_table, acc_table);
        }
    } break;
    case GGML_OP_MEAN:
    case GGML_OP_ARGMAX:
    ...
    case GGML_OP_REPEAT: {
        ...
        if (src0->grad) {
            src0->grad = ggml_add_or_set(
                ctx, src0->grad, ggml_repeat_back(ctx, tensor->grad, src0->grad), zero_table, acc_table);
        }
    } break;
    case GGML_OP_REPEAT_BACK: {
        if (src0->grad) {
    ...
    case GGML_OP_RMS_NORM: {
        ...
        if (src0->grad) {
            ...
            src0->grad = ggml_add_or_set(
                ctx, src0->grad, ggml_rms_norm_back(ctx, src0, tensor->grad, eps), zero_table, acc_table);
        }
    } break;
    case GGML_OP_RMS_NORM_BACK: {
        GGML_ABORT("fatal error"); // TODO: not implemented
    ...
    case GGML_OP_SCALE: {
        ...
        if (src0->grad) {
            ...
            src0->grad =
                ggml_add_or_set(ctx, src0->grad, ggml_scale_impl(ctx, tensor->grad, s, false), zero_table, acc_table);
        }
    } break;
    case GGML_OP_SET: {
        const size_t nb1 = ((int32_t *)tensor->op_params)[0];
        ...
        if (src0->grad || src1->grad) {
            ...
            tensor_grad_view = ggml_view_4d(
                ctx, tensor->grad, src1->ne[0], src1->ne[1], src1->ne[2], src1->ne[3], nb1, nb2, nb3, offset);
        }
        ...
        if (src0->grad) {
            src0->grad = ggml_add_or_set(
                ctx,
                src0->grad,
                ggml_acc_impl(ctx, tensor->grad, ggml_neg(ctx, tensor_grad_view), nb1, nb2, nb3, offset, false),
                zero_table,
                acc_table);
        }
    ...
    case GGML_OP_RESHAPE: {
        ...
        if (src0->grad) {
            src0->grad = ggml_add_or_set(
                ...
                src0->grad,
                ggml_reshape(
                    ctx, ggml_is_contiguous(tensor->grad) ? tensor->grad : ggml_cont(ctx, tensor->grad), src0->grad),
                zero_table,
                acc_table);
        }
    } break;
    ...
    case GGML_OP_TRANSPOSE: {
        // necessary for llama
        if (src0->grad) {
            src0->grad = ggml_add_or_set(ctx, src0->grad, ggml_transpose(ctx, tensor->grad), zero_table, acc_table);
        }
    } break;
    case GGML_OP_GET_ROWS: {
    ...
    case GGML_OP_DIAG_MASK_ZERO: {
        ...
        if (src0->grad) {
            const int n_past = ((int32_t *)tensor->op_params)[0];
            src0->grad = ggml_add_or_set(
                ctx, src0->grad, ggml_diag_mask_zero_impl(ctx, tensor->grad, n_past, false), zero_table, acc_table);
        }
    } break;
    case GGML_OP_SOFT_MAX: {
        // necessary for llama
        if (src0->grad) {
            src0->grad =
                ggml_add_or_set(ctx, src0->grad, ggml_soft_max_back(ctx, tensor->grad, tensor), zero_table, acc_table);
        }
    ...
    case GGML_OP_FLASH_ATTN_EXT: {
        ...
        if (src0->grad) {
            ...
            src0->grad = ggml_add_or_set(ctx, src0->grad, grad_q, zero_table, acc_table);
        }
        if (src1->grad) {
            ...
            src1->grad = ggml_add_or_set(ctx, src1->grad, grad_k, zero_table, acc_table);
        }
        if (src2->grad) {
            ...
            src2->grad = ggml_add_or_set(ctx, src2->grad, grad_v, zero_table, acc_table);
        }
    } break;
    case GGML_OP_FLASH_ATTN_BACK: {
    ...
    case GGML_OP_UNARY: {
        ...
        case GGML_UNARY_OP_ABS: {
            if (src0->grad) {
                src0->grad = ggml_add_or_set(
                    ctx, src0->grad, ggml_mul(ctx, ggml_sgn(ctx, src0), tensor->grad), zero_table, acc_table);
            }
        } break;
        case GGML_UNARY_OP_SGN: {
            if (src0->grad) {
        ...
        case GGML_UNARY_OP_RELU: {
            if (src0->grad) {
                src0->grad = ggml_add_or_set(
                    ctx, src0->grad, ggml_mul(ctx, ggml_step(ctx, src0), tensor->grad), zero_table, acc_table);
            }
        } break;
        case GGML_UNARY_OP_SIGMOID: {
            GGML_ABORT("fatal error"); // TODO: not implemented
        ...
        case GGML_UNARY_OP_SILU: {
            ...
            if (src0->grad) {
                src0->grad =
                    ggml_add_or_set(ctx, src0->grad, ggml_silu_back(ctx, src0, tensor->grad), zero_table, acc_table);
            }
        } break;
        case GGML_UNARY_OP_EXP: {
            if (src0->grad) {
                src0->grad =
                    ggml_add_or_set(ctx, src0->grad, ggml_mul(ctx, tensor, tensor->grad), zero_table, acc_table);
            }
        } break;
        default:
            GGML_ABORT("fatal error");
    ...
    case GGML_OP_CROSS_ENTROPY_LOSS: {
        if (src0->grad) {
            src0->grad = ggml_add_or_set(
                ctx, src0->grad, ggml_cross_entropy_loss_back(ctx, src0, src1, tensor->grad), zero_table, acc_table);
        }
...
void ggml_build_opt_adamw(struct ggml_context *ctx,
                          struct ggml_cgraph *gf,
                          struct ggml_cgraph *gb,
                          float alpha,
                          float beta1,
...
struct ggml_cgraph *ggml_new_graph_custom(struct ggml_context *ctx, size_t size, bool grads) {
    ...
    struct ggml_tensor **grads_ptr =
        grads ? incr_ptr_aligned(&p, size * sizeof(struct ggml_tensor *), sizeof(struct ggml_tensor *)) : NULL;
            ...
            if (ggml_bitnet_can_mul_mat(node->src[0], node->src[1], node)) {
                ...
                if (vec_dot_type == GGML_TYPE_I8_S) {
                    cur = ggml_row_size(vec_dot_type, ggml_nelements(node->src[1]))
                          + node->src[1]->ne[1] * sizeof(float) + node->src[1]->ne[1] * sizeof(int32_t);
                } else {
                    cur = ggml_row_size(vec_dot_type, ggml_nelements(node->src[1]));
                }
            }
...
ggml_threadpool_new_impl(struct ggml_threadpool_params *tpp, struct ggml_cgraph *cgraph, struct ggml_cplan *cplan) {
...
static void ggml_graph_export_leaf(const struct ggml_tensor *tensor, FILE *fout) {
    ...
    fprintf(fout,
            "%-6s %-12s %8d %" PRId64 " %" PRId64 " %" PRId64 " %" PRId64 " %16zu %16zu %16zu %16zu %16p %32s\n",
    ...
    // populate result
    {
        ...
        // leafs
        {
            ...
            for (uint32_t i = 0; i < n_leafs; ++i) {
                ...
                fprintf(
                    stderr, "%s: loaded leaf %u: '%16s', %9zu bytes\n", __func__, i, tensor->name, ggml_nbytes(tensor));
            }
        }
...
static void ggml_graph_dump_dot_node_edge(
    FILE *fp, const struct ggml_cgraph *gb, struct ggml_tensor *node, struct ggml_tensor *parent, const char *label) {
    ...
    for (int i = 0; i < gb->n_leafs; i++) {
        ...
        if (ggml_nelements(node) < 5 && node->data != NULL) {
            ...
            for (int j = 0; j < ggml_nelements(node); j++) {
                ...
                } else if (node->type == GGML_TYPE_F32 || node->type == GGML_TYPE_F16 || node->type == GGML_TYPE_BF16) {
...
static enum ggml_opt_result linesearch_backtracking(const struct ggml_opt_params *params,
                                                    int nx,
                                                    float *x,
                                                    float *fx,
                                                    float *g,
...
static enum ggml_opt_result ggml_opt_lbfgs(struct ggml_context *ctx,
    ...
    while (true) {
        ...
        ls = linesearch_backtracking(
            &params, nx, x, &fx, g, d, step, xp, f, gb, &cplan, np, ps, &cancel, callback, callback_data);
        if (cancel) {
            return GGML_OPT_RESULT_CANCEL;
        }
...
GGML_API void
ggml_opt_init(struct ggml_context *ctx, struct ggml_opt_context *opt, struct ggml_opt_params params, int64_t nx) {
    opt->ctx = ctx;
    opt->params = params;
    opt->iter = 0;
    opt->nx = nx;
    ...
    if (opt->ctx == NULL) {
        ...
        if (opt->params.type == GGML_OPT_TYPE_ADAM) {
            ...
            ctx_opt_params.mem_size =
                ...
                + ggml_type_size(GGML_TYPE_F32) * (nx * 5 + opt->params.lbfgs.m * 2 + nx * opt->params.lbfgs.m * 2);
            if (opt->params.past > 0) {
                ctx_opt_params.mem_size +=
                    GGML_MEM_ALIGN + ggml_tensor_overhead() + ggml_type_size(GGML_TYPE_F32) * opt->params.past;
            }
    ...
    case GGML_OPT_TYPE_ADAM: {
        ...
        opt->adam.pf = params.past > 0 ? ggml_new_tensor_1d(opt->ctx, GGML_TYPE_F32, params.past) : NULL;
    ...
    case GGML_OPT_TYPE_LBFGS: {
        ...
        opt->lbfgs.pf = params.past > 0 ? ggml_new_tensor_1d(opt->ctx, GGML_TYPE_F32, params.past) : NULL;
    ...
    // check the magic before making allocations
    {
        ...
        for (uint32_t i = 0; i < sizeof(magic); i++) {
            if (magic[i] != GGUF_MAGIC[i]) {
                fprintf(stderr,
                        "%s: invalid magic characters '%c%c%c%c'\n",
                        __func__,
                        magic[0],
                        magic[1],
                        magic[2],
        ...
        for (uint64_t i = 0; i < n_kv; ++i) {
            ...
            case GGUF_TYPE_ARRAY: {
                ...
                case GGUF_TYPE_BOOL: {
                    ...
                    ok = ok
                         && gguf_fread_el(
                             file, kv->value.arr.data, kv->value.arr.n * gguf_type_size(kv->value.arr.type), &offset);
                } break;
                case GGUF_TYPE_STRING: {
                    // prevent from integer overflow in the malloc below
    ...
    // compute the total size of the data section, taking into account the alignment
    {
        ...
        for (uint64_t i = 0; i < ctx->header.n_tensors; ++i) {
            ...
            if (ggml_blck_size(info->type) == 0 || ne % ggml_blck_size(info->type) != 0) {
                fprintf(stderr,
                        "%s: tensor '%s' of type %d (%s) number of elements (%" PRId64
    ...
    for (uint32_t i = 0; i < src->header.n_kv; i++) {
        ...
        case GGUF_TYPE_ARRAY: {
            if (src->kv[i].value.arr.type == GGUF_TYPE_STRING) {
                ...
                gguf_set_arr_data(ctx,
                                  src->kv[i].key.data,
                                  src->kv[i].value.arr.type,
                                  src->kv[i].value.arr.data,
